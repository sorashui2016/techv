...
export default async function SourcesPage() {
  ...
  try {
    sources = await prisma.source.findMany({ orderBy: { createdAt: "desc" } });
  } catch (error) {
              ...
              {sources.map((source) => (
                <tr key={source.id} className="border-t border-zinc-100">
                  <td className="px-4 py-3">
                    ...
                    <div className="max-w-sm truncate text-xs text-zinc-500">{source.url}</div>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div>{source.lastCheckStatus ?? "未知"}</div>
                    {source.lastCheckError ? (
                      <div className="max-w-xs truncate text-xs text-rose-600">{source.lastCheckError}</div>
                    ) : null}
                  </td>
                  <td className="px-4 py-3">
